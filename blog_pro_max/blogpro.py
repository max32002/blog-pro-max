...
VERSION = "1.0.44"
VERSION_HISTORY = [
    {"version": "1.0.44", "date": "2026-04-02", "changes": "分析流程重構：全部分析一次執行完畢，文章本文存 keyword.md，所有分析存 keyword_analysis.md；HTML 輸出自動合併文章＋分析為單一 .html；修正 Gemini CLI Skill Mode 重複寫入 bug：LLM 模式改為全部分析完畢後一次性寫檔，禁止多次 write_file；新增腳本失敗停止指令，防止靜默回退 LLM 模式造成內容重疊"},
