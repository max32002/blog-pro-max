...
VERSION = "1.0.45"
VERSION_HISTORY = [
    {"version": "1.0.45", "date": "2026-04-02", "changes": "分析流程重構：全部分析一次執行完畢，文章本文存 keyword.md，所有分析存 keyword_analysis.md；HTML 輸出自動合併文章＋分析為單一 .html；修正 Gemini CLI Skill Mode 重複寫入 bug：LLM 模式改為全部分析完畢後一次性寫檔，禁止多次 write_file；新增腳本失敗停止指令，防止靜默回退 LLM 模式造成內容重疊"},
...
5. 文章生成後，**依以下順序執行全部分析，完成後一次性存檔**（不需詢問）：
   ...
   **腳本執行方式（具備腳本能力時）：**
   ...
   > ⚠️ 若腳本執行失敗，顯示錯誤並停止，不要改用 LLM 模式重新生成。
...
5. 回報結果與檔案位置（路徑相對於使用者工作目錄）
   ...
   > ⚠️ **若腳本執行失敗**（例如缺少 API 金鑰或環境問題），請直接顯示錯誤訊息並停止。
   > **不要改用 LLM 模式重新生成文章**，以避免與腳本已寫入的部分內容重疊。
