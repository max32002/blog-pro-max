...
5. 文章生成後，**依以下順序執行全部分析，完成後一次性存檔**（不需詢問）：
   ...
   **腳本執行方式（具備腳本能力時）：**
   ...
   ```
   腳本會輸出 `[SYSTEM]` 與 `[USER]` 提示詞，以及目標輸出路徑。
   > ✅ 腳本成功後：**依照腳本輸出的 `[SYSTEM]` + `[USER]` 提示詞，由 LLM 生成文章**，並將內容寫入腳本指定的輸出路徑。
   > ❌ 腳本執行失敗（非零退出碼）：顯示錯誤訊息並停止。
...
3. 執行腳本取得生成提示詞：
   ...
   - `[SYSTEM]`：系統角色提示詞
   - `[USER]`：文章生成指令
   - 目標輸出路徑（`output/關鍵字.md`）

   > ❌ 若腳本執行失敗（非零退出碼），顯示錯誤訊息並停止。

4. **依照腳本輸出的 `[SYSTEM]` + `[USER]` 提示詞，由 LLM 生成完整文章**（只生成文章，不做分析）
5. 將文章寫入腳本指定的輸出路徑，然後執行全部分析（三維度審稿、時事趨勢、標題、封面、發散、唱反調、插話、插畫、問題、迷因），並儲存 `output/關鍵字_analysis.md` 與合併 `output/關鍵字.html`
6. 回報結果與檔案位置（路徑相對於使用者工作目錄）
